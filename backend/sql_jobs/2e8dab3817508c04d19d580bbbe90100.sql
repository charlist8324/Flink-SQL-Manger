...
SELECT `db_source_order`.`id`, `db_source_order`.`product`, `db_source_order`.`order`, `db_source_user`.`name`, `db_source_user`.`sex` FROM `db_source_order`
INNER JOIN `db_source_user` ON `db_source_order`.`id` = `db_source_user`.`id`;
