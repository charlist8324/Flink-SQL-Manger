SET 'parallelism.default' = '1';
SET 'execution.checkpointing.interval' = '30000ms';
SET 'execution.checkpointing.mode' = 'EXACTLY_ONCE';
SET 'execution.checkpointing.dir' = 'file:///home/flink-1.18.1/checkpoint/checkpoints';
SET 'table.exec.state.ttl' = '24h';

CREATE TABLE `db_source_address` (
    `id` INT,
    `address` STRING,
    `phone` STRING,
    PRIMARY KEY (`id`) NOT ENFORCED
) WITH (
    'connector' = 'mysql-cdc',
    'hostname' = '192.168.31.251',
    'port' = '3306',
    'database-name' = 'db_source',
    'table-name' = 'address',
    'username' = 'root',
    'password' = 'Admin@900',
    'server-time-zone' = 'Asia/Shanghai',
    'scan.incremental.snapshot.enabled' = 'true',
    'scan.startup.mode' = 'initial'
);

CREATE TABLE `db_source_order` (
    `id` INT,
    `product` STRING,
    `order` STRING,
    PRIMARY KEY (`id`) NOT ENFORCED
) WITH (
    'connector' = 'mysql-cdc',
    'hostname' = '192.168.31.251',
    'port' = '3306',
    'database-name' = 'db_source',
    'table-name' = 'order',
    'username' = 'root',
    'password' = 'Admin@900',
    'server-time-zone' = 'Asia/Shanghai',
    'scan.incremental.snapshot.enabled' = 'true',
    'scan.startup.mode' = 'initial'
);

CREATE TABLE `db_source_user` (
    `id` INT,
    `name` STRING,
    `sex` STRING,
    PRIMARY KEY (`id`) NOT ENFORCED
) WITH (
    'connector' = 'mysql-cdc',
    'hostname' = '192.168.31.251',
    'port' = '3306',
    'database-name' = 'db_source',
    'table-name' = 'user',
    'username' = 'root',
    'password' = 'Admin@900',
    'server-time-zone' = 'Asia/Shanghai',
    'scan.incremental.snapshot.enabled' = 'true',
    'scan.startup.mode' = 'initial'
);

CREATE TABLE `db_source_address_sink` (
    `id` INT,
    `address` STRING,
    `phone` STRING,
    `name` STRING,
    `sex` STRING,
    PRIMARY KEY (`id`) NOT ENFORCED
) WITH (
    'connector' = 'jdbc',
    'url' = 'jdbc:mysql://192.168.31.234:3306/target',
    'driver' = 'com.mysql.cj.jdbc.Driver',
    'table-name' = 'db_source_address_sink',
    'username' = 'root',
    'password' = 'Admin@900'
);

CREATE TABLE `db_source_order_sink` (
    `id` INT,
    `product` STRING,
    `order` STRING,
    PRIMARY KEY (`id`) NOT ENFORCED
) WITH (
    'connector' = 'jdbc',
    'url' = 'jdbc:mysql://192.168.31.234:3306/target',
    'driver' = 'com.mysql.cj.jdbc.Driver',
    'table-name' = 'db_source_order_sink',
    'username' = 'root',
    'password' = 'Admin@900'
);

CREATE TABLE `db_source_user_sink` (
    `id` INT,
    `name` STRING,
    `sex` STRING,
    PRIMARY KEY (`id`) NOT ENFORCED
) WITH (
    'connector' = 'jdbc',
    'url' = 'jdbc:mysql://192.168.31.234:3306/target',
    'driver' = 'com.mysql.cj.jdbc.Driver',
    'table-name' = 'db_source_user_sink',
    'username' = 'root',
    'password' = 'Admin@900'
);

BEGIN STATEMENT SET;

INSERT INTO `db_source_address_sink`
SELECT `db_source_address`.`id`, `db_source_address`.`address`, `db_source_address`.`phone`, `db_source_user`.`name`, `db_source_user`.`sex` FROM `db_source_address`
INNER JOIN `db_source_user` ON `db_source_address`.`id` = `db_source_user`.`id`;

INSERT INTO `db_source_order_sink`
SELECT `id`, `product`, `order` FROM `db_source_order`;

INSERT INTO `db_source_user_sink`
SELECT `id`, `name`, `sex` FROM `db_source_user`;

END;
