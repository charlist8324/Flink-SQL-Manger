...
CREATE TABLE `db_source_address_sink` (
    `phone` STRING,
    `ph` BIGINT,
    PRIMARY KEY (`phone`) NOT ENFORCED
) WITH (
    'connector' = 'jdbc',
    'url' = 'jdbc:mysql://192.168.31.234:3306/target',
    'driver' = 'com.mysql.cj.jdbc.Driver',
    'table-name' = 'db_source_address_sink',
    'username' = 'root',
    'password' = 'Admin@900'
);
...
CREATE TABLE `db_source_order_sink` (
    ...
    'url' = 'jdbc:mysql://192.168.31.234:3306/target',
    'driver' = 'com.mysql.cj.jdbc.Driver',
    'table-name' = 'db_source_order_sink',
    'username' = 'root',
    'password' = 'Admin@900'
);
...
CREATE TABLE `db_source_user_sink` (
    ...
    'url' = 'jdbc:mysql://192.168.31.234:3306/target',
    'driver' = 'com.mysql.cj.jdbc.Driver',
    'table-name' = 'db_source_user_sink',
    'username' = 'root',
    'password' = 'Admin@900'
);
...
SELECT `phone`, COUNT(`phone`) AS `ph`
FROM `db_source_address`
GROUP BY `phone`;

INSERT INTO `db_source_order_sink`
SELECT `db_source_order`.`id`, `db_source_order`.`product`, `db_source_order`.`order`, `db_source_address`.`address`, `db_source_address`.`phone` FROM `db_source_order`
INNER JOIN `db_source_address` ON `db_source_order`.`id` = `db_source_address`.`id`;
